...
--callback 交互回调
--txtsize 字体大小
function QueryDialog:ctor(msg, callback, txtsize, queryType, showMannual, mannualUpdateUrl)
	queryType = queryType or QueryDialog.QUERY_SURE_CANCEL
    --showMannual = showMannual or false
	self._callback = callback
	self._canTouchOutside = true
	self._mannualUpdateUrl = mannualUpdateUrl

	local this = self 
	self:setContentSize(1280,720)
	self:move(0,720)
	...
	cc.Label:createWithTTF(msg, "fonts/round_body.ttf", not txtsize and QueryDialog.DEF_TEXT_SIZE or txtsize)
		...
		:setTextColor(cc.c3b(242,216,92))
...
function QueryDialog:dismiss()
	...
	self:runAction(
		cc.Sequence:create(
			cc.MoveTo:create(0.3,cc.p(0,720)),
