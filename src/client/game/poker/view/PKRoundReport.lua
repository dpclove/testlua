...
local PKRoundReport = class("PKRoundReport", function()
	return cc.Layer:create()
end)

function PKRoundReport:ctor(roomPlayers, playerSeatIdx, rptMsgTbl, roomid, isLast, localHeadImg, isFangZuobi)
	...
	local csbNode = cc.CSLoader:createNode("XiaojieLayer.csb")
   ...
   Text_status:setVisible(false)
    ...
    gt.seekNodeByName(csbNode,"Text_dateAndTime1"):setString(os.date("%H:%M:%S"))
...
function PKRoundReport:screenshotShareToWX()
...
function PKRoundReport:onNodeEvent(eventName)
...
function PKRoundReport:onTouchBegan(touch, event)
	return true

end

function PKRoundReport:update()
...
function PKRoundReport:pushShareCodeImage(authCode)
	gt.log("===================HY", authCode, type(authCode))
end
return PKRoundReport
