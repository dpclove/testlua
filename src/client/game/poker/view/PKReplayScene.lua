...
local PKReplayScene = class("PKReplayScene", function()
	return cc.Layer:create()
end)


PKReplayScene.ColorType = {
    -- 大小王
    KING                = 0x40,
    -- 黑桃
    HEITAO              = 0x30,
...
function PKReplayScene:is_changzhu(card)
...
function PKReplayScene:checkColor(card)
    if self.isChangZhu then
        if card >= 78 or card == 2 or card == 18 or card == 34 or card == 50 then
            --大小王和四个2
            return PKReplayScene.ColorType.KING
        elseif card >= 49 and card <= 61 then
            --黑桃
            return PKReplayScene.ColorType.HEITAO
        elseif card >= 33 and card <= 45 then
            --红桃
            return PKReplayScene.ColorType.HONGTAO
        elseif card >= 17 and card <= 29 then
            --梅花
            return PKReplayScene.ColorType.MEIHUA
        elseif card >= 1 and card <= 13 then
            --方片
            return PKReplayScene.ColorType.FANGPIAN
        end
    else
        if card >= 78 then
            --大小王
...
function PKReplayScene:checkColors(card)
...
--手牌排序
function PKReplayScene:sortCards(handcards, MainCard)
    local heitaoCards = {}
    local hongtaoCards = {}
    local meihuacards = {}
    local fangkuaicards = {}
    ...
    if MainCard == PKReplayScene.ColorType.HEITAO then
    ...
    elseif MainCard == PKReplayScene.ColorType.HONGTAO then
        for i=1, #maincards do
            table.insert(allcards, maincards[i])
        end    
        for i=1, #hongtaoCards do
    ...
    elseif MainCard == PKReplayScene.ColorType.MEIHUA then
        for i=1, #maincards do
            table.insert(allcards, maincards[i])
        end     
        for i=1, #meihuacards do
    ...
    elseif MainCard == PKReplayScene.ColorType.FANGPIAN then
        for i=1, #maincards do
            table.insert(allcards, maincards[i])
        end  
        for i=1, #fangkuaicards do
...
function PKReplayScene:ctor(replayData, type)
	-- 注册节点事件
	...
	-- 加载界面资源
	local csbNode = cc.CSLoader:createNode("PKReplayScene.csb")
...
function PKReplayScene:inidUI(replayData)
    -- 播放按键
	local playBtn = gt.seekNodeByName(self.rootNode, "Btn_play")
	-- 暂停
	local pauseBtn = gt.seekNodeByName(self.rootNode, "Btn_pause")
    ...
    for i=1, 5 do
        local seatIdx = (i-1 + self.seatOffset) % 5 + 1
...
function PKReplayScene:addImage(headSpr,args)
    args = args or false
...
function PKReplayScene:onNodeEvent(eventName)
...
function PKReplayScene:onTouchBegan(touch, event)
	return true
end

-- 快推的话,原理是将牌恢复到最初始状态
-- 然后快速行进到当前状态
function PKReplayScene:preRound()
...
-- 快速回合播放
function PKReplayScene:nextRound()
	-- -- 如果暂停或者已经结束,是否需要回退
    self.curReplayStep = self.curReplayStep + 5
...
function PKReplayScene:doAction( curReplayStep )
...
function PKReplayScene:update(delta)
...
function PKReplayScene:setPause(isPause)
    self.isPause =  isPause
end

function PKReplayScene:updateCurrentTime()
end

function PKReplayScene:qiangZhuang(oper)
    
    local seatIdx = (oper[1] + self.seatOffset) % 5 + 1
	local playerNode = gt.seekNodeByName(self.rootNode, "Node_player" .. seatIdx)
...
function PKReplayScene:xuanZhu(oper)
    for i=1, 5 do
	    local playerNode = gt.seekNodeByName(self.rootNode, "Node_player" .. i)
	    local Img_score = gt.seekNodeByName(playerNode, "Img_score")
    ...
    local seatIdx = (self.zhuangPos + self.seatOffset) % 5 + 1
	local playerNode = gt.seekNodeByName(self.rootNode, "Node_player" .. seatIdx)
	local Img_banker = gt.seekNodeByName(playerNode, "Img_banker")
    ...
    elseif oper[4][1] == 64 then
          gt.log("64______________")
          Image_zhupai:setVisible(false)
          gt.seekNodeByName(self.rootNode, "Image_zhupai_0"):setVisible(true)
          self.isChangZhu = false
    end
    ...
    for i=1, 5 do
        ...
        for j=1, #handcards do
            ...
            if self:checkColor(handcards[j]) == self.MainColor or self:checkColor(handcards[j]) == PKReplayScene.ColorType.KING  and self.MainColor ~= 64 then
...
function PKReplayScene:maiDi(oper)
    local seatIdx = (self.zhuangPos + self.seatOffset) % 5 + 1
	local playerNode = gt.seekNodeByName(self.rootNode, "Node_player" .. seatIdx)
	local Img_banker = gt.seekNodeByName(playerNode, "Img_banker")
    Img_banker:setVisible(true)
...
function PKReplayScene:xuanFuZhuang(oper)
    local seatIdx = (self.zhuangPos + self.seatOffset) % 5 + 1
	local playerNode = gt.seekNodeByName(self.rootNode, "Node_player" .. seatIdx)
	local Img_banker = gt.seekNodeByName(playerNode, "Img_banker")
    Img_banker:setVisible(true)
    
    local zhuangpos = (self.zhuangPos + self.seatOffset) % 5 + 1
    local fuzhuangpos = (oper[3] + self.seatOffset) % 5 + 1
...
function PKReplayScene:chuPai(oper)
    for i=1, 5 do
	    local playerNode = gt.seekNodeByName(self.rootNode, "Node_player" .. i)
	    local Img_score = gt.seekNodeByName(playerNode, "Img_score")
        Img_score:setVisible(false)
...
function PKReplayScene:onFinish(oper)
