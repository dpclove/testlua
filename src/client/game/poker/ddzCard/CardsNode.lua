...
function CardsNode:onTouchBegan(touch, event)
	...
	if gt.actionTouch ~= nil and gt.actionTouch == false then
		return
	end

	-- if not self.controllerTouch then
	-- 	return
	-- end
	...
	if nil ~= self.m_beginSelectCard 
		...
		--选牌效果
		--gt.soundEngine:Poker_playEffect("sound_res/ddz/touch_my_card.mp3")
		...
		if  self.m_pSelectedListener and self.m_pSelectedListener.nil_card  then self.m_pSelectedListener:nil_card()  end
	end
...
--	if #vecSelectCard == 0 and self.m_pSelectedListener and self.m_pSelectedListener.nil_card  then self.m_pSelectedListener:nil_card()  end
    ...
    if self.m_pSelectedListener  and  self.m_pSelectedListener.reSetBankeeCard then self.m_pSelectedListener:reSetBankeeCard() end

	-- self.controllerTouch = false
	-- local seqAction = cc.Sequence:create(cc.DelayTime:create(1),cc.CallFunc:create(function()
	-- 		self.controllerTouch = true
	-- 	end))
...
function CardsNode:updateCardsNode( cards, isShowCard, bAnimation, pCallBack) -- ppp
	...
	if ((3 == self.m_nViewId) 
		or (1 == self.m_nViewId)
		or (gt.MY_VIEWID == self.m_nViewId))
...
function CardsNode:updateCardsData( cards )
	...
	if #vecChildren ~= #cards then
		--gt.log("children count " .. #vecChildren .. " cards count " .. #cards)
		return
	end
...
-- 加牌
function CardsNode:addCards( addCards, handCards ,bankerCardDatd)
	...
	if tmpcount > gt.card_num_max then
		print("超出最大牌数")
		return
	end
	...
	local buf = {}
	if bankerCardDatd then 
		buf = addCards
	end 

	self.m_cardsCount = tmpcount
	self:arrangeAllCards(true,nil,buf,#addCards)
	self.m_cardsCount = gt.card_num_max 
end


function CardsNode:getSprCard()
		...
		for i = 1, self.m_cardsCount do
			if i <= 10 then
				...
				if tmp then
					tmp:setCardValue(cbCardData)
					tmp:setScale(CARD_LEFT_SCALE)
					local pos = cc.p((index - center) * CARD_X_DIS * CARD_HIDE_SCALE, -35)
					local moveTo = cc.MoveTo:create(0.5 + index / 16, pos)
					local call = cc.CallFunc:create(function ()
						tmp:showCardBack(false)
					end)
					local spa = cc.Spawn:create(moveTo, call)
...
function CardsNode:reSet()
	self:dragCards(self:filterDragSelectCards(false))
end

-- 重置
function CardsNode:reSetCards(bool)
	self.m_beginSelectCard = nil
	self.m_endSelectCard = nil

	self:dragCards(self:filterDragSelectCards(false),bool)
	self.m_mapSelectedCards = {}
	self.m_mapDragSelectCards = {}
...
-- 提示弹出
-- @param[cards] 提示牌
function CardsNode:suggestShootCards( cards ,bool)
	if type(cards) ~= "table"  then
		return
	end
...
function CardsNode:arrangeAllCards( showAnimation, pCallBack ,bankerCardDatd , addcards)
	local idx = 0;
	if showAnimation then
		...
		if gt.MY_VIEWID == self.m_nViewId then
			local num = 5
			if count >=17 then 
				CARD_X_DIS = 1130/(count-1)
				if count == 21 or count == 20 then 
					num = 0
				end
			else
				CARD_X_DIS = 1130/(17-1)
			end
			...
			for i = 1, count do
				...
				if nil ~= tmp then
					...
					local pos = cc.p((i - center) * CARD_X_DIS-num, 0)
					tmp:stopAllActions()
					if tmp:getDispatched() then
						tmp:setPosition(pos)
						gt.log("getDispatched___________________")
					else
						tmp:setDispatched(true)
   						if bankerCardDatd then 
							for j = 1, #bankerCardDatd  do
									if  tmp:getCardData() == bankerCardDatd[j] then 
										gt.log("youmieyou+___________________")
											 pos =  cc.p((i - center) * CARD_X_DIS, 20)
									end
							end
						end
						

						gt.log("getDispatched___________________s",i,count)
						...
						local hideBack = cc.CallFunc:create(function ()
							...
							if i == count then
								...
								gt.log("cc_______")
								self.m_bDispatching = false		
								if nil ~= self.m_pSelectedListener and nil ~= self.m_pSelectedListener.xuanzepuke then 
									self.m_pSelectedListener.xuanzepuke = {}
									gt.log("null_______________")
								end					
							end
							gt.log("type........",tonumber(addcards))
							if type(addcards) == "number" then 
								gt.log("idx.......",idx)
								if idx == addcards then 
									self.m_bDispatching = false
									
									if nil ~= self.m_pSelectedListener and nil ~= self.m_pSelectedListener.xuanzepuke then 
										self.m_pSelectedListener.xuanzepuke = {}
										gt.log("null_______________1111")
									end
								end
							end			
			...
			for i = 1, count do
				...
				if nil ~= tmp then
					...
					if tmp:getDispatched() then
						tmp:setPosition(pos)
						gt.log("getDispatched___________________1")
					else
						tmp:setDispatched(true)
						gt.log("getDispatched___________________2")
						local moveTo = cc.MoveTo:create(ANI_BEGIN, pos)
						local delay = cc.DelayTime:create(ANI_BEGIN)
						...
						-- self.controllerTouch = false
						local seq = cc.Sequence:create(delay, showBack,cc.CallFunc:create(function()
							-- self.controllerTouch = true
							end))
...
function CardsNode:set_CardShoot(card,bool)
...
function CardsNode:dragCards( vecCard ,bool)
