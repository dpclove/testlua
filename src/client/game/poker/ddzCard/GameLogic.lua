...
function GameLogic:GetCardLogicValue(nCardData)
    ...
    return nCardValue <= 2 and (nCardValue + 13) or nCardValue --err
end
...
function GameLogic:GetCardType(cbCardData, cbCardCount)
    ...
    elseif cbCardCount == 2 then    --对牌火箭
		...
		if GameLogic:GetCardLogicValue(cbCardData[1]) == 18 then
			if cbCardData[2] == 0x4F or cbCardData[2] == 0x4E or GameLogic:GetCardLogicValue(cbCardData[2]) == 15 then
				return GameLogic.CT_ERROR
			end
		end 
		if GameLogic:GetCardLogicValue(cbCardData[2]) == 18 then
			if cbCardData[1] == 0x4F or cbCardData[1] == 0x4E or GameLogic:GetCardLogicValue(cbCardData[1]) == 15 then
				return GameLogic.CT_ERROR
			end
		end 
		...
		if GameLogic:GetCardLogicValue(cbCardData[1]) == 18 or  GameLogic:GetCardLogicValue(cbCardData[2]) == 18 then
            return GameLogic.CT_DOUBLE
        end
    end
    ...
    if tagAnalyseResult[1][4] > 0 then
		...
		if (tagAnalyseResult[1][4] == 1 and cbCardCount == 8) and tagAnalyseResult[1][2] == 1 and
			tagAnalyseResult[1][1] == 1 and tagAnalyseResult[1][5] > 0 then
			if cbCardData[1][1] ~= 0x4F and cbCardData[1][1] ~= 0x4E and GameLogic:GetCardLogicValue(cbCardData[1]) ~= 15 then
				return GameLogic.CT_FOUR_TAKE_TWO
			end 
        end
        return GameLogic.CT_ERROR
	...
	--有癞子
	if tagAnalyseResult[1][5] > 0  then--默认只有一个癞子的算法逻辑
		if tagAnalyseResult[1][3] > 0 then 
			...
			if (tagAnalyseResult[1][3] == 1 and cbCardCount == 8) and tagAnalyseResult[1][2] == 2 then
				return GameLogic.CT_FOUR_TAKE_TWO
			end
		end
	end 
