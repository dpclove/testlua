...
function Charge.buy(type_, id_)
	...
	mt.chargeTable = require("client/game/Purchase/Recharge")[mt.itemid]
	...
	local getPayResult = function ()
		...
		if string.len(ret) > 0 and mt.checkPayResult then
			...
			if event == "PAY_SUCCESS" then
				gt.log("PAY_SUCCESS_11")
				-- Charge.requestPayFromServer()
				gt.log("PAY_SUCCESS_22")
				--购买成功
				local money = cc.UserDefault:getInstance():getIntegerForKey("money"..tostring(gt.playerData.uid), 0)
				cc.UserDefault:getInstance():setIntegerForKey("money"..tostring(gt.playerData.uid), money + mt.chargeTable["Cost"])
                gt.dispatchEvent(gt.EventType.APPSTORE_REFRESH_CARD_COUNT)
...
function Charge.requestPayFromServer()
		...
		if mt.xhr.readyState == 4 and (mt.xhr.status >= 200 and mt.xhr.status < 207) then
			...
			if response.code == 0 then
				...
				if result == 0 or result == "0" then
					...
					-- local luaBridge = require("cocos/cocos2d/luaoc")
					-- local ok, ret = luaBridge.callStaticMethod("AppController", "getTransactionId")
					-- if ok and string.len(ret) > 0 then
					-- 	gt.log("TransactionId=="..ret)
					-- 	gt.finishDeliver(ret)
					-- end					
					-- require("app/views/NoticeTips"):create("购买成功", "购买成功，请查收", nil, nil, true)
				else
					gt.log("-----购买失败")
					Charge.closeMaskLayer()
	...
	local unionid = gt.socketClient:getPlayerUUID()
	local playerType = "P"
	local payURL = string.format("%s?receipt=%s&serverCode=%s&userId=%s&unionId=%s&playerType=%s&isPurchaseLimit=%s&productNumber=%s&payWay=%s", gt.payUrl, chargeData, gt.serverCode, gt.playerData.uid, unionid, playerType, limitState, productId, gt.sdkBridge.payWay)
	gt.dump("----验证url" .. payURL)
