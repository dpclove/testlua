...
function Sound:ctor()
	...
	local soundEftPercent = cc.UserDefault:getInstance():getIntegerForKey("Sound_Eft_Percent", 80)
...
-- @return 音效id
-- end --
function Sound:playEffect(fileName, isLoop, test, fileSuffix)
		--gt.log("-----------------------------audio_button_click")
	-- 关闭声音并且非循环
	if not fileName or string.len(fileName) == 0 then
		...
		--gt.log("-----------------------------audio_button_click")
	local fileSuffix = fileSuffix or "mp3"
...
function Sound:Poker_playEffect(fileName, isLoop, test, fileSuffix)
		--gt.log("-----------------------------audio_button_click")
	-- 关闭声音并且非循环
	if not fileName or string.len(fileName) == 0 then
		return
	...
	local filePath = string.format("sfx/%s.%s", fileName, fileSuffix)
	isLoop = isLoop or false
	local audioID = ccexp.AudioEngine:play2d(fileName, isLoop, self.soundEffectVolume * 0.01)
...
function Sound:playMusic_poker(fileName, isLoop)
	if not fileName or string.len(fileName) == 0 then
		return
	end
	...
	isLoop = isLoop or false
	self:stopMusic()
	self.musicAudioID = ccexp.AudioEngine:play2d(fileName, isLoop, self.musicVolume * 0.01)
	return self.musicAudioID
end
