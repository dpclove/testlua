...
class Xfer(object):
    ...
    def initDirs(self):
        items = configAssets_release.Paths.items()
    ...
    for f in fl:
        if os.path.isdir(os.path.join(path,f)): # if is a dir.
            ...
            if f != '.svn' and f != '.DS_Store' and f != 'version.manifest' and f != 'project.manifest':
                ...
                configAssets_release.assetsXml += "\n\t\t\"%s\" : {\n\t\t\t\"md5\" : \"%s\"\n\t\t}, " % (prefix + '/' + f, md5)
    return configAssets_release.assetsXml
    ...
    for f in fl:
        if os.path.isdir(os.path.join(path,f)): # if is a dir.
            ...
            if  f != '.svn' and f != '.DS_Store' and f != 'version.manifest' and f != 'project.manifest':
                ...
                configAssets_release.assetsMoreXml += "\"%s\" : {\"md5\" : \"%s\"}, " % (prefix + '/' + f, md5)
    return configAssets_release.assetsMoreXml
...
def createVersionFile(oPath, versionNumber, versionString):
    versionXml = configAssets_release.getVersion(versionNumber, versionString)
...
def createProjectFile(oPath, versionNumber, versionString, newassets):
    projectXml = configAssets_release.getProject(versionNumber, versionString, newassets)
...
if __name__ == "__main__":
    ...
    # 拷贝src和res到release
    versionNumber = configAssets_release.VersionNumber
    versionString = configAssets_release.versionString
    operationPath = configAssets_release.ReleasePath + '/' + configAssets_release.ProjectName
    ...
    #新的md5
    configAssets_release.assetsMoreXml = ''
